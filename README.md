...
| **一次录像推翻了「已收敛」的结论** | 实践 9 的 `mean_reward` 由负转正（−0.75 → 6.98），但真实跟踪误差在恶化（0.932 → 1.834）。根因是参考动作的关节列序是**广度优先**、而模型是深度优先——用正运动学交叉验证定位（拿关节角驱动模型、比对数据自带的刚体位置），修正后正运动学误差 **0.178 m → 0.0011 m**，关节超限 11/29 → 1/29。**重训验证：误差由持续恶化转为稳定收敛（0.938 → 0.732），且只用了此前 40% 的轮数** |
...
scripts/     训练/录像/验收等可复现的操作脚本
docs/        实践记录：做了什么、结果如何、问题怎么定位的
teaching/    原理讲解：为什么这么做、背后的数学与代码是怎么回事
```

### `docs/` 与 `teaching/` 的分工

两个目录写的是同一批实践，但**读者和目的不同**：

| | `docs/`（实践文档） | `teaching/`（教学文档） |
|---|---|---|
| 读者 | 招聘方、GitHub 访客、课程评分 | 面试前的自己 |
| 回答 | **做了什么、结果如何** | **为什么这样做、原理是什么** |
| 内容 | 实现规格、实测数据、排查过程、评分细则对照 | 从零讲起的概念、公式推导、代码逐行走读 |
| 写法 | 结论先行，数据说话 | 假设读者零基础，不跳步 |
| 典型章节 | 「逐条核对官方规格」「训练完成后的效果核查」 | 「问题从哪来」「核心概念（零基础可懂）」 |

举例：同样是实践 10 的 RayCaster，
- `docs/实践10` 写：射线网格 17×17=289，观测维 289×8=2312，与规格一致
- `teaching/10` 写：射线测高是什么原理、为什么要打 289 条而不是更多、
  两套同名但无关的 `offset` 分别是什么意思

**想从零学这套技术**，从 [`teaching/README.md`](teaching/README.md) 进 ——
那里有 6152 行的 RL 原理讲解（从"什么是状态、动作、奖励"讲起）、
11 份按实践的教学文档、以及一份按面试问题类型组织的横向题库。
