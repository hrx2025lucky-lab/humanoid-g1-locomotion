...
11 个实践的完整源代码与最终策略权重在 [`code/`](code/)。每个实践目录下的
`MODIFIED.md` 逐文件列出哪些是我写的、哪些来自上游框架或课程包——先读它，再看代码。

训练日志、虚拟环境、机器人网格资产等未收录，说明见 [`code/README.md`](code/README.md)。

## 动态预览与完整视频

已收录实践2—11的31段完整MP4，以及实践3的2段补充动画。实践08的录像是复核记录而非通过成果，标注随附；其余实践可从动画预览或带标题的链接打开完整文件。
...
让机器人既走得准又走得像人。走得准靠速度跟踪奖励，走得像人靠一个判别器：它同时看人类参考动作和机器人动作，尽力分辨谁是谁，机器人则想办法骗过它，骗过的程度折算成风格奖励。

主要工作是排查步态为什么不自然，做了十轮单因素实验，每轮的判定标准都在训练开始前写死存档。先定位到判别器被稳定性保护全程暂停——500次更新里判定饱和500次，只真正训练了4次，拖腿由此而来；解除后抬脚从2.13cm提到5.01cm。

之后找到两个有效改动。一是**把判别器的梯度惩罚从10提到50—100**，摆臂方向终于修正，手臂与同侧腿反向摆动，相位相关从+0.85（同手同脚）变为−0.70/−0.84，并在新随机种子下复现。二是**新增一项直接惩罚双支撑时长的奖励**，走走停停的节奏明显改善，该占比由44%降到22%，落入人类走路区间，同时顺带把慢走抬脚从2.5cm提到5.5cm。

没有采用框架自带的接触时序奖励，因为实测它可以被"一直站着"套利——启用后双支撑反而升高。

仍未通过验收。现在的阻碍是航向与左右对称性：双支撑降下来会让单脚支撑期变长，航向跟着变差，二者相关系数−0.849；把航向权重提上去又会让双支撑回升、摆臂相位再度转正。三者互相耦合，最新一轮有17/20个场景判定卡在左右摆动时长比这一项。

[![实践08：摆臂方向已修正的候选，侧面](practices/08_amp_locomotion/media/preview_armswing_fixed.gif)](practices/08_amp_locomotion/media/p8_armswing_fixed_side_20s.mp4)

预览为前8秒截取，完整20.00秒见下方MP4

- [摆臂方向已修正 · 侧面](practices/08_amp_locomotion/media/p8_armswing_fixed_side_20s.mp4) · 20.00秒
- [摆臂方向已修正 · 正面](practices/08_amp_locomotion/media/p8_armswing_fixed_front_20s.mp4) · 20.00秒
- [此前候选（摆臂为同手同脚）· 正面](practices/08_amp_locomotion/media/p8_gate_pass_front_20s.mp4) · 20.00秒
...
补全深度处理链路后，课程示例策略在 MuJoCo 平地连续走完 20 秒、13.87 米，骨盆稳定在 0.71 米以上，说明整条链路正确。我们自己训练的 5000 模型则几乎不前进：平地 1.43 米、粗糙地形 1.24 米。

我一开始以为这是跨仿真损失，扫描指令后发现不是。同一场景下课程策略的跟踪比是 85%—114%，我们的模型在任何指令下只有 10%—21%，在训练允许的最大指令处落差依然存在。回头看原生评估，它的 XY 速度 RMSE 是 0.2604，与均速 0.2572 几乎相等——**这个策略在 Isaac 里本来就没学会跟踪速度**，不是迁移把它弄坏的。
