...
已完成速度、转向与左右脚接触时序的量化比较。6200阶段模型存在左右摆腿不协调、短促摆动和速度起伏，**尚未通过自然行走验证**，因此暂停首页动态预览。

[查看步态诊断、阶段录像与完整结果](practices/08_amp_locomotion/README.md)
...
已完成双指令控制、两类蒸馏、长动作跟踪、地形配对和平台路径的分项验证。分层导航已有1,536局配对结果，尚未得到随机布局训练的总体优势；AMP已完成速度、转向与步态诊断，拟人走跑仍未通过验证，深度策略尚未达到完整跑酷表现。
