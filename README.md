...
主要工作是排查步态为什么不自然，做了八轮单因素实验，每轮的判定标准都在训练开始前写死存档。先定位到判别器被稳定性保护全程暂停——500次更新里判定饱和500次，只真正训练了4次，拖腿由此而来；解除后抬脚从2.13cm提到5.01cm。之后手臂不摆的问题排除了四条路径，最后发现有效的是**把判别器的梯度惩罚从10提到50—100**：摆臂方向终于修正，手臂与同侧腿反向摆动，相位相关从+0.85（同手同脚）变为−0.70/−0.84，并在新随机种子下复现。

仍未解决的是走走停停：双支撑占比44%，正常人走路是20%–25%，所以画面上还能看出顿挫。0.3m/s慢走也仍未通过，因为参考动作库里最长的连续慢走只有0.40秒，属于数据缺口。
