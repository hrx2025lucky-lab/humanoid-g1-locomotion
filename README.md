...
**技术栈**：Isaac Lab（训练）+ MuJoCo（跨仿真验证）

**方法**：PPO

**这一项做了什么**：策略导出为ONNX，核对关节顺序映射、动作尺度、PD参数与观测历史的跨仿真一致性
...
**这一项做了什么**：高度扫描传感器接入观测，修正相对地形高度与非有限射线，台阶与方块地形行走
...
**技术栈**：Isaac Lab + MuJoCo（部署）

**方法**：HoST预训练策略 · PD控制

**这一项做了什么**：23关节G1上复现增量目标角接口与历史观测拼接，仰躺起身到站立
...
**这一项做了什么**：速度与骨盆高度双指令任务：高度指令采样、观测拼接、高度跟踪奖励与消融对照
...
**方法**：分层强化学习：高层PPO + 冻结低层行走策略

**这一项做了什么**：高层输出速度指令驱动冻结低层，固定布局与随机布局模型的同预算交叉评估
...
**方法**：PPO + 教师学生蒸馏（Action Matching / KL Matching）

**这一项做了什么**：两类蒸馏损失在同一动作库上的对照：动作回归与高斯分布KL散度
...
**技术栈**：GMR（重定向）+ SMPL-X（人体模型）

**方法**：逆运动学，无强化学习

**这一项做了什么**：人体动作到G1的坐标系、四元数约定、关节顺序、帧率与运动库格式转换
...
**方法**：PPO + AMP对抗式动作先验（判别器提供风格奖励）

**这一项做了什么**：G1 29自由度，速度跟踪与风格奖励混合，判别器饱和保护的根因定位与步态量化评估
...
**方法**：PPO + BeyondMimic方法

**这一项做了什么**：失败统计驱动的参考采样、相位推进与参考坐标对齐，长时间全身轨迹跟踪
...
**这一项做了什么**：高度扫描感知平台地形，世界锚点奖励尺度对照，全身人物交互动作跟踪
...
**技术栈**：Project Instinct + MuJoCo（Sim2Sim）

**方法**：PPO + 运动风格奖励

**这一项做了什么**：深度图历史编码与延迟建模，ONNX导出，深度感知跑酷策略的跨仿真执行链路
