...
> **训练状态（2026-09-08 更新）**：代码 3 个 TODO 已全部补完，
> 离线审计 **24/24 通过**，并且**已完成整机训练**：
> 先跑 3000 轮，看到误差仍在快速下降后从 `model_2999.pt` 续训到 ~9000 轮。
>
> | 指标（五段均值） | 首 → 末 | 读法 |
> |---|---|---|
> | `error_joint_pos` | 1.126 → **1.042** | 跟踪精度在提升 |
> | `error_body_pos` | 0.115 → **0.108** | 同上 |
> | `mean_episode_length` | 1111 → **1263** | 活得更久 |
> | `Curriculum/progress` | 0.299 → **0.857** | 课程推进到 86% |
> | `push_level` | 2.09 → **6.00** | 抗扰动等级顶格 |
>
> **注意最后两行**：误差下降的同时，课程难度也在大幅上升
> （推力等级从 2.09 涨到顶格 6.00）。
> 「在更难的条件下误差还更小」才是真实的能力提升 ——
> 只看误差数字会低估这个结果。
...
8. *这个训练跑出来了吗？*
   跑出来了。先跑 3000 轮，看到 `error_joint_pos` 仍在快速下降
   （末段降 30%），判断是"练到一半被叫停"而不是"练到头了"，
   于是从 `model_2999.pt` 续训到约 9000 轮。

   结果：`error_joint_pos` 1.126→1.042、`error_body_pos` 0.115→0.108、
   存活时长 1111→1263。

   **但只报误差是不够的** —— 同期 `push_level`（抗扰动课程）
   从 2.09 涨到顶格 6.00、`progress` 从 0.299 到 0.857。
   也就是说：**在难得多的条件下，误差反而更小**。
   如果只看"误差从 1.126 降到 1.042、只降了 7%"，会严重低估这个结果。

   > 这也是我在这个项目里反复强调的一点：
   > 评价训练效果必须把**课程难度**一起看，否则会把
   > "任务变难导致的误差上升"误判成"训练失败"。
