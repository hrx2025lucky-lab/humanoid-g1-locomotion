...
def randomize_mixed_obstacle_layout(
    ...
    # NAV_RESAMPLE_EVERY：每 N 次 reset 才真正重排一次布局（默认 1 = 每次都排，
    # 保持课程原行为）。
    #
    # 动机：sample_layout() 是三层嵌套的 Python 循环
    #   4096 env × 120 槽位 × max_resample_tries(256) ≈ 1.26 亿次迭代，CPU 单线程。
    # 而本函数是 mode="reset"，每个 episode 结束都要跑一遍，实测单轮
    # 30.6s（baseline 4.7s，慢 6.5 倍），3000 轮要 25 小时。
    #
    # 降低重排频率不改变"障碍布局会变"这个难度来源，只是让同一份布局
    # 多服役几个 episode: 对高层策略而言，它仍然会在训练过程中见到
    # 大量不同布局，只是不再每局都换。
    interval = int(os.environ.get("NAV_RESAMPLE_EVERY", 1))
    if interval > 1:
        counter = getattr(env, "_obstacle_resample_counter", 0)
        env._obstacle_resample_counter = counter + 1
        if counter % interval != 0:
            # 跳过重排，但仍要 write_to_sim: reset 会把刚体位姿复位，
            # 不重写的话障碍会回到默认位置（而不是保持上一次的布局）。
            layout.write_to_sim(env_ids)
            return
