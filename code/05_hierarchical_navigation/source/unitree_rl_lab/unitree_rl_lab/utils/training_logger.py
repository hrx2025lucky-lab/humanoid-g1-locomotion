...
# rsl-rl 3.1.2 没有 rsl_rl.utils.logger.Logger: 该模块是更新版本才引入的，
# 本机装的是 3.1.2。UnitreeLogger 的作用仅是把自适应采样指标打印得更可读，
# 训练逻辑全部走 OnPolicyRunner 父类。因此缺它时降级为标准 runner，
# 让训练照常进行（控制台输出没那么好看，TensorBoard 指标不受影响）。
try:
    from rsl_rl.utils.logger import Logger

    _HAS_RSL_RL_LOGGER = True
except ImportError:  # rsl-rl < 该 Logger 引入版本
    Logger = object  # type: ignore[assignment,misc]
    _HAS_RSL_RL_LOGGER = False
...
class UnitreeOnPolicyRunner(OnPolicyRunner):
    ...
    def __init__(self, env, train_cfg: dict, log_dir: str | None = None, device: str = "cpu") -> None:
        ...
        if not _HAS_RSL_RL_LOGGER:
            # 降级路径：保留父类自带的日志器，训练与 TensorBoard 记录不受影响。
            return
