...
# 原作者机器上的硬编码路径改为环境变量 + 本机默认值，
# 与主仓库 repos/unitree_rl_lab 的做法保持一致。
UNITREE_MODEL_DIR = os.environ.get(
    "UNITREE_MODEL_DIR", os.path.expanduser("~/unitree_model")
)
UNITREE_ROS_DIR = os.environ.get(
    "UNITREE_ROS_DIR", os.path.expanduser("~/unitree_ros")
)
...
UNITREE_G1_29DOF_CFG = UnitreeArticulationCfg(
    # 本机没有 G1 的 USD 资产（原作者机器上的 unitree_model 目录未随仓库提供），
    # 改用 unitree_ros 里的 URDF: 与主仓库 repos/unitree_rl_lab 的做法一致，
    # 实践 2 的训练用的就是这条路径。
    spawn=UnitreeUrdfFileCfg(
