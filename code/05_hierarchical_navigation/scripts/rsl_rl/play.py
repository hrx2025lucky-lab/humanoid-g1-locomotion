...
# handle_deprecated_rsl_rl_cfg 是上游为老格式 cfg 准备的兼容垫片，
# 新版 isaaclab_rl 已经把它删掉了，硬导入会让 play.py 直接
# ImportError 起不来（本仓库的训练能跑、回放跑不了，就是这个原因）。
# 垫片的作用是把过时字段翻译成新字段；我们的 cfg 本来就是新格式，
# 没有它就是恒等变换，所以缺失时退化成 no-op 是安全的。
try:
