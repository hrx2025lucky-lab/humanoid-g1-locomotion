...
class TeacherActor(MLPModel):
  ...
  def __init__(
    ...
    if self.obs_dim != expected_obs_dim:
      raise ValueError(
        "TeacherActor observation dimension mismatch: "
  ...
  def as_onnx(self, verbose: bool = False) -> nn.Module:
    return OnnxTeacherActor(self, verbose=verbose)
...
class TeacherCritic(MLPModel):
  ...
  def __init__(
    ...
    if self.obs_dim <= self.motion_obs_dim:
      raise ValueError(
        "TeacherCritic expects privileged critic observations after "
...
class OnnxTeacherActor(nn.Module):
  """ONNX wrapper for TeacherActor."""

  is_recurrent: bool = False

  def __init__(self, model: TeacherActor, verbose: bool = False) -> None:
    super().__init__()
