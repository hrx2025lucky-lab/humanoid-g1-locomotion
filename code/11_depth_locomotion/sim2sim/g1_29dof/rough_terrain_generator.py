#!/usr/bin/env python3
"""Generate MuJoCo rough terrain scenes for sim2sim depth-camera tests.

The default configuration recreates the terrain patches currently used by
scene_rough.xml: pyramid stairs, box grid, pyramid stairs with gaps, slope pyramid,
and random heightfield.
Edit rough_terrain_cfg.py or pass a different output path on the command line.
"""
...
class RoughTerrainGenerator:
    ...
    def add_pyramid_stairs_with_gaps(self, cfg: PyramidStairsWithGapsCfg) -> None:
        ...
        self.worldbody.append(
            ET.Comment(
                f" {cfg.name}: pyramid stairs with vertical square gaps, centered at {vec(cfg.center)}, "
                f"width={fmt(cfg.width)}, length={fmt(cfg.length)}. "
            )
        )
