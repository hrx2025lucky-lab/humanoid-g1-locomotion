...
class DepthImagePipeline:
    ...
    def show_normalize_image(self, image: np.ndarray):
        ...
        vis_image = (vis_image - self.near_clip) / (
    ...
    def append(self, image: np.ndarray):
        # image processing pipeline: resize -> crop -> inpaint -> gaussian blur -> clip and normalize
        # resize image
        h, w = self.shape # h: 目标高度, w: 目标宽度
        # dsize is (width, height), which is the reverse of self.shape = (h, w)
        image = cv2.resize(image, dsize=(w, h), interpolation=cv2.INTER_NEAREST)

        # crop image
        y1, y2, x1, x2 = self.crop_region # y1/y2：从图像上方/下方裁掉的像素数；x1/x2：从左侧/右侧裁掉的像素数
        # y2/x2 are pixel counts removed from the bottom/right, and either may be
        # zero, so the end index must be written as size - margin. Using image[y1:-y2]
        # would return an empty array whenever y2 == 0, which is the configured case.
        ...
        # gaussian blur
        # sigmaY must be passed by keyword: the positional slot after sigmaX is dst.
        ...
        # resample delay for next frame
        self.resample_delay()


    def resample_delay(self):
        """Randomly sample a delay in [min_delay, max_delay]."""
    ...
    def get_depth_obs(self):
        ...
        for index in self.indices:
            # Apply delay: shift index backward by self.delay frames
...
class Sim2simInstance:
    ...
    def _key_callback(self, keycode: int):
        ...
        self.velocity_commands[0] = np.clip(
            self.velocity_commands[0], *COMMAND_RANGES["lin_vel_x"]
        )
        self.velocity_commands[1] = np.clip(
            self.velocity_commands[1], *COMMAND_RANGES["lin_vel_y"]
        )
        self.velocity_commands[2] = np.clip(
            self.velocity_commands[2], *COMMAND_RANGES["ang_vel_z"]
        )
        print(
            "Command "
    ...
    def update_observation(self):
        ...
        # self.depth_pipeline.show_normalize_image(image)
