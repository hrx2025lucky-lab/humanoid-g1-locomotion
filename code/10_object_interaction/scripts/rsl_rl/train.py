...
# 这个转换逻辑原本在 train.py 和 helpers/play_helpers.py 各有一份。
# 昨天只修了这里的私有副本，play.py 用的公共版没动，于是训练能跑、
# 回放仍然 KeyError: 'policy',而截图脚本照样拍下了一张锁屏当"材料"。
# 重复实现就是这么漏的，统一复用公共版。
sys.path.insert(0, os.path.join(os.path.dirname(os.path.abspath(__file__))))
from helpers import convert_legacy_policy_cfg as _convert_legacy_policy_cfg
...
def main(env_cfg: ManagerBasedRLEnvCfg | DirectRLEnvCfg | DirectMARLEnvCfg, agent_cfg: RslRlOnPolicyRunnerCfg):
    ...
    runner_cfg = _convert_legacy_policy_cfg(agent_cfg.to_dict())
