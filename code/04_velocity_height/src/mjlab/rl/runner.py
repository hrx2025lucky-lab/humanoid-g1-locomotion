...
class MjlabOnPolicyRunner(OnPolicyRunner):
  ...
  def load(
    ...
    3. Restore the adaptive learning-rate scalar when loading the optimizer.
    """
    ...
    if load_cfg is None or load_cfg.get("optimizer", False):
      # PPO.load restores Adam's lr, but PPO.update uses its own scalar and
      # writes that back to Adam. Keep both states consistent across resumes.
