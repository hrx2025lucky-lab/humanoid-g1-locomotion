...
class GeneralMotionRetargeting:
    ...
    def retarget(self, human_data, offset_to_ground=False):
        ...
        if self.use_ik_match_table1:
            ...
            vel1 = mink.solve_ik(
                self.configuration, self.tasks1, dt=dt, solver=self.solver,
                damping=self.damping, safety_break=True, limits=self.ik_limits
            )
        ...
        if self.use_ik_match_table2:
            ...
            dt = self.configuration.model.opt.timestep
            vel2 = mink.solve_ik(
                self.configuration, self.tasks2, dt=dt, solver=self.solver,
                damping=self.damping, safety_break=True, limits=self.ik_limits
            )
