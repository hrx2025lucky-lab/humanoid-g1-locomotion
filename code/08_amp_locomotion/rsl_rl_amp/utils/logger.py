...
import rsl_rl_amp
from rsl_rl_amp.utils.log_writer import LogWriter
from rsl_rl_amp.utils.utils import resolve_callable
...
class Logger:
    ...
    def __init__(
        ...
        self.device = device
        self.git_status_repos = [rsl_rl_amp.__file__]
        ...
        self.reward_return_buffers: dict[str, deque] = {}
        self.reward_step_buffers: dict[str, deque] = {}
        self.cur_reward_component_sums: dict[str, torch.Tensor] = {}
    ...
    def init_logging_writer(self) -> None:
        ...
        .. note::
            ...
            any additional constructor kwargs (see :class:`~rsl_rl_amp.utils.LogWriter`). The plain string aliases
        ...
        if self.writer is not None:
            ...
            if reward_components is not None:
                for name, values in reward_components.items():
                    if name not in self.cur_reward_component_sums:
                        self.cur_reward_component_sums[name] = torch.zeros_like(self.cur_reward_sum)
                        self.reward_return_buffers[name] = deque(maxlen=100)
                        self.reward_step_buffers[name] = deque(maxlen=100)
                    self.cur_reward_component_sums[name] += values

            # Clear data for completed episodes
            new_ids = (dones > 0).nonzero(as_tuple=False)
            self.rewbuffer.extend(self.cur_reward_sum[new_ids][:, 0].cpu().numpy().tolist())
            self.lenbuffer.extend(self.cur_episode_length[new_ids][:, 0].cpu().numpy().tolist())
            for name, sums in self.cur_reward_component_sums.items():
                completed_returns = sums[new_ids][:, 0]
                completed_lengths = self.cur_episode_length[new_ids][:, 0]
                self.reward_return_buffers[name].extend(completed_returns.cpu().numpy().tolist())
                self.reward_step_buffers[name].extend(
                    (completed_returns / completed_lengths.clamp_min(1.0)).cpu().numpy().tolist()
                )
                sums[new_ids] = 0
    ...
    def log(
        ...
        if self.writer is not None:
            ...
            if len(self.rewbuffer) > 0:
                ...
                for name, values in self.reward_return_buffers.items():
                    if values:
                        self.writer.add_scalar(f"Train/mean_{name}_return", statistics.mean(values), it)
                        self.writer.add_scalar(
                            f"Train/mean_{name}_reward_step",
                            statistics.mean(self.reward_step_buffers[name]),
                            it,
                        )
            ...
            if len(self.rewbuffer) > 0:
                ...
                for name, values in self.reward_return_buffers.items():
                    if values:
                        log_string += (
                            f"{f'Mean {name} return:':>{pad}} {statistics.mean(values):.2f}\n"
                            f"{f'Mean {name} reward/step:':>{pad}} "
                            f"{statistics.mean(self.reward_step_buffers[name]):.4f}\n"
                        )
