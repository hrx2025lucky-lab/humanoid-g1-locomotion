...
def resolve_callable(callable_or_name: type | Callable | str) -> Callable:
    ...
    - Simple name: for example ``"PPO"`` or ``"ActorCritic"`` (searched within ``rsl_rl_amp``).
    ...
    Raises:
        ...
        ValueError: If a simple name cannot be found in rsl_rl_amp packages.
    """
    ...
    # Simple name - look for it in rsl_rl_amp
    for _, module_name, _ in pkgutil.iter_modules(rsl_rl_amp.__path__, "rsl_rl_amp."):
        ...
        if default_set_name not in obs_groups:
            if default_set_name in obs:
                ...
                raise ValueError(
                    ...
                    f" Please refer to `rsl_rl_amp.utils.resolve_obs_groups()` for information on how to configure the"
