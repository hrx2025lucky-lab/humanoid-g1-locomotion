...
from rsl_rl_amp.extensions import RandomNetworkDistillation, Symmetry, resolve_rnd_config, resolve_symmetry_config
from rsl_rl_amp.models import MLPModel
from rsl_rl_amp.storage import RolloutStorage
from rsl_rl_amp.utils import compile_model, resolve_callable, resolve_obs_groups, resolve_optimizer
...
class PPO:
    ...
    def update(self) -> dict[str, float]:
        ...
        for batch in generator:
            ...
            # L = L_policy + c_v * L_value - c_e * H(pi)
            #
            # 熵项前的负号是关键：优化器最小化 loss，而我们希望最大化熵
            # （熵越大动作分布越分散、探索越充分）。写成 + 会让策略迅速塌缩成
            # 确定性动作，AMP 里表现为步态僵死、判别器轻易识破。
            loss = (
                surrogate_loss
    ...
    def compile(self, mode: str | None = None) -> None:
        ...
        See :func:`~rsl_rl_amp.utils.compile_model` for the set of accepted modes.
