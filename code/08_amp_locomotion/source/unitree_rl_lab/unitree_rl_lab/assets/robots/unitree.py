...
UNITREE_MODEL_DIR = os.environ.get(
    "UNITREE_MODEL_DIR", str(Path.home() / "unitree_rl_lab-main" / "unitree_model")
)
# 默认值 ~/unitree_ros 在本机不存在，实际仓库在 Roxan_warmup/repos 下。
# 与实践 2/5 使用的主仓库保持同一路径，避免两边加载到不同版本的 G1 描述。
