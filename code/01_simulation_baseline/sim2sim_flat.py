...
import torch
import yaml
ROOT = os.environ.get('ROBOT_WORKSPACE', os.path.expanduser('~/workspace/robotics_workspace'))
TRAIN_RUN_DIR = os.environ.get('RL_LAB_RUN_DIR', os.path.join(ROOT, 'repos/unitree_rl_lab/logs/rsl_rl/unitree_g1_29dof_velocity/2026-08-31_10-49-33'))
ROBOT_SCENE = os.environ.get('MUJOCO_SCENE', os.path.join(ROOT, 'repos/unitree_mujoco/unitree_robots/g1/scene_29dof.xml'))
POLICY_PATH = os.path.join(TRAIN_RUN_DIR, 'exported', 'policy.pt')
DEPLOY_CONFIG = os.path.join(TRAIN_RUN_DIR, 'params', 'deploy.yaml')
SIM_DT = 0.002
DECIMATION = 10
DEVICE = 'cpu'
INIT_BASE_POS = (0.0, 0.0, 0.8)
INIT_BASE_QUAT_WXYZ = (1.0, 0.0, 0.0, 0.0)
COMMAND_STEP = (0.2, 0.1, 0.2)
COMMAND_RANGES = {'lin_vel_x': (-0.5, 1.0), 'lin_vel_y': (-0.3, 0.3), 'ang_vel_z': (-0.2, 0.2)}
VIEWER_DISTANCE = 4.0
...
class FlatSim2Sim:
    OBS_ORDER = ('base_ang_vel', 'projected_gravity', 'velocity_commands', 'joint_pos_rel', 'joint_vel_rel', 'last_action')
    ...
    def __init__(self, args):
        ...
        self.default_joint_pos_asset = np.asarray(self.cfg['default_joint_pos'], dtype=np.float32)
        ...
        if missing:
            raise ValueError(f'deploy.yaml 缺少观测项 {missing}。\n实际有的观测项：{list(self.obs_cfg.keys())}')
        extra = [n for n in self.obs_cfg if n not in self.OBS_ORDER]
        if extra:
            raise ValueError(f'deploy.yaml 里有本脚本未处理的观测项 {extra}。\n若含 height_scan，说明这是粗糙地形策略，请用实践 2 的脚本。')
    ...
    def _sensor_adr(self, name: str, fallback_name: str | None=None) -> int:
        ...
        if sid < 0:
            names = [mujoco.mj_id2name(self.model, mujoco.mjtObj.mjOBJ_SENSOR, i) for i in range(self.model.nsensor)]
    ...
    def reset(self):
        ...
        self.history = {name: deque([np.zeros(self._term_dim(name), dtype=np.float32)] * self._hist_len(name), maxlen=self._hist_len(name)) for name in self.OBS_ORDER}

    def _term_dim(self, name: str) -> int:
        return {'base_ang_vel': 3, 'projected_gravity': 3, 'velocity_commands': 3, 'joint_pos_rel': self.num_joints, 'joint_vel_rel': self.num_joints, 'last_action': self.num_joints}[name]
    ...
    def update_observation(self):
        ...
        projected_gravity = quat_apply_inverse(imu_quat, np.array([0.0, 0.0, -1.0], dtype=np.float32))
        joint_pos_asset = joint_pos_sdk[self.asset_to_sdk]
        joint_vel_asset = joint_vel_sdk[self.asset_to_sdk]
        raw = {'base_ang_vel': imu_gyro, 'projected_gravity': projected_gravity, 'velocity_commands': self.command.copy(), 'joint_pos_rel': joint_pos_asset - self.default_joint_pos_asset, 'joint_vel_rel': joint_vel_asset, 'last_action': self.last_action.copy()}
    ...
    def key_callback(self, keycode: int):
        k = {'up': key_code('KEY_UP', 265), 'down': key_code('KEY_DOWN', 264), 'left': key_code('KEY_LEFT', 263), 'right': key_code('KEY_RIGHT', 262), 'space': key_code('KEY_SPACE', 32), 'w': key_code('KEY_W', 87), 's': key_code('KEY_S', 83), 'r': key_code('KEY_R', 82)}
        ...
        if keycode == k['up']:
            ...
            print('[reset]')
            return
        lo = np.array([COMMAND_RANGES['lin_vel_x'][0], COMMAND_RANGES['lin_vel_y'][0], COMMAND_RANGES['ang_vel_z'][0]], dtype=np.float32)
        hi = np.array([COMMAND_RANGES['lin_vel_x'][1], COMMAND_RANGES['lin_vel_y'][1], COMMAND_RANGES['ang_vel_z'][1]], dtype=np.float32)
    ...
    def print_info(self):
        ...
        print(f'  policy_dt     {self.decimation * self.sim_dt:.4f} s (deploy.yaml 记录 {self.step_dt:.4f} s)')
        if abs(self.decimation * self.sim_dt - self.step_dt) > 1e-06:
        ...
        try:
            while True:
                ...
                if raw_action.shape[0] != self.num_joints:
                    raise ValueError(f'策略输出维度 {raw_action.shape[0]} != 关节数 {self.num_joints}')
                ...
                if self.no_viewer and n % 10 == 0:
                    z = self.data.qpos[2]
                    print(f'step {n:5d}  base_z={z:.3f}  vx={self.data.qvel[0]:+.3f}  |action|={np.abs(raw_action).max():.3f}')
