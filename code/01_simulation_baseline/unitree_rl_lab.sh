...
if ! [[ -z "${VIRTUAL_ENV}" ]]; then
    ...
    echo "[Error] No Python environment activated. Please activate the Isaac Lab venv or conda environment first."
    exit 1
fi
...
case "$1" in
    -i|--install)
        ...
        ${python_exe} -m pip install -e ${UNITREE_RL_LAB_PATH}/source/unitree_rl_lab/
