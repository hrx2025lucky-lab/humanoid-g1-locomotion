...
## 附、训练完成后的效果核查（2026-09-07）

跑满 2999 轮，`model_2999.pt` 已存盘。**但不能只看 reward** ——
实践 9 的教训是数值指标可以自洽地错着，所以逐项查了真实任务指标。

### 速度跟踪误差确实在变差

| 指标 | 五段走势 | |
|---|---|---|
| `error_vel_xy` | 0.197→0.376→0.495→0.485→**0.509** | ↑ 2.59× |
| `error_vel_yaw` | 0.544→0.884→1.308→1.190→**1.239** | ↑ 2.28× |
| `bad_orientation` 终止 | 0.992→0.809→0.238→0.136→**0.128** | ↓ 摔倒大幅减少 |
| `time_out` 终止 | 0.005→0.191→0.762→0.864→**0.872** | ↑ 能撑满整段 |

### 有多少能归因于课程升难度

```
lin_vel_cmd_levels   1.20 → 1.59    指令难度 1.33×
error_vel_xy         0.197 → 0.509  跟踪误差 2.59×
归一化误差（误差/难度）  0.164 → 0.320  仍然翻倍
```

**归一化之后误差还是涨了一倍，所以不能全归因于课程。**

### 但这次是 AMP 的设计权衡，不是 bug

查奖励混合配置：

```python
# rsl_rl_ppo_cfg.py:92
"task_reward_weight": 0.4

# amp.py:165  r_t = (1 - alpha) * r_amp + alpha * r_task
```

**风格奖励占 60%，任务奖励只占 40%。** AMP 的核心思想就是牺牲一部分
任务精度去换动作的拟人度 —— 速度跟得没那么准，但步态像人。
这是**有意的设计**，不是实现出错。

判别器工作正常，佐证机制在按预期运行：

```
amp_expert_score          稳定在 +0.70
amp_discriminator_score   稳定在 -0.70
amp_gradient_penalty      0.005~0.018（小而稳，没有梯度爆炸）
```

专家与策略的分数分离清晰且**全程稳定**，说明判别器既没有过强
（把策略压死）也没有崩溃（GAN 常见的失败模式）。

### 与实践 9 的对比 —— 同样是"误差变差"，性质完全不同

| | 实践 9 | 实践 8 |
|---|---|---|
| 现象 | error_joint_pos 1.161→1.900 | error_vel_xy 0.197→0.509 |
| 奖励项状态 | 饱和为 3e-07，**梯度消失** | 正常，只是权重占 40% |
| 判别器/对照 | — | expert vs policy 分离稳定 |
| 摔倒率 | 未改善 | **0.992→0.128 大幅改善** |
| 性质 | **bug**：超参与数据不匹配 | **设计权衡**：精度换风格 |

**判据是"这一项还有没有梯度"。** 实践 9 的 `motion_joint_pos` 奖励是
1e-18 量级，策略再怎么改善都拿不到反馈；实践 8 的任务奖励虽然只占 40%，
但那 40% 是实打实在起作用的 —— 证据是摔倒率从 99% 降到 13%。

> 如果只看"误差在涨"这一个现象，两者会被归成同一类问题。
> **要判断是 bug 还是权衡，得看奖励函数在当前误差量级上还有没有梯度。**
