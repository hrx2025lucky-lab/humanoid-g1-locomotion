...
import numpy as np
ROXAN_ROOT = os.environ.get('ROBOT_WORKSPACE', '/path/to/workspace')
SIM2SIM_PKG = os.environ.get('HW2_SIM2SIM_DIR', os.path.join(ROXAN_ROOT, 'shenlan_hw/hw2_sim2sim/sim2sim'))
UPRIGHT_FAIL_GZ = -0.7
...
def load_app(run_dir: str | None, policy: str | None):
    ...
    import config
    from sim2sim_raycaster import RaycasterSim2Sim
    if run_dir:
    ...
    for path in (config.POLICY_PATH, config.DEPLOY_CONFIG, config.ROBOT_SCENE, config.RAYCASTER_PLUGIN_LIBRARY):
...
def report_obs_contract(app) -> None:
    ...
    print(f'  控制频率: policy_dt={app.policy_dt:.3f}s (decimation={app.decimation} × sim_dt={app.policy_dt / app.decimation:.3f}s)')
    print()
...
def report_segments(segments: list[Segment], fell: bool, total_steps: int, policy_dt: float) -> bool:
    print('── ② 指令跟踪（机体系）──')
    print(f"  {'指令段':<12}{'cmd vx':>8}{'实际 vx':>9}{'误差':>8}{'cmd yaw':>9}{'实际 yaw':>10}{'净位移 m':>10}")
    ok = True
    for seg in segments:
        ...
        print(f'  {seg.name:<12}{seg.command[0]:>8.2f}{vx:>9.3f}{err:>8.3f}{seg.command[2]:>9.2f}{vyaw:>10.3f}{disp:>10.3f}')
    ...
    if all_gz:
        print(f'  投影重力 z      均值 {np.mean(all_gz):+.3f}  最差 {max(all_gz):+.3f}  （直立 = -1.0，> {UPRIGHT_FAIL_GZ} 判为摔倒）')
    if all_cl:
...
def main() -> int:
    ...
    segments = [Segment('站立', 1.5, (0.0, 0.0, 0.0)), Segment('前进', args.seconds, (args.vx, 0.0, 0.0)), Segment('前进+转向', args.seconds, (args.vx, 0.0, args.vyaw)), Segment('停止', 1.5, (0.0, 0.0, 0.0))]
