...
ROOT = os.environ.get('HW4_LOG_DIR', '/path/to/workspace/shenlan_hw/hw4_mjlab/logs/rsl_rl/g1_velocity_height')
GROUPS = [('baseline', 'ablation_baseline', '三段齐全'), ('blind_actor', 'ablation_blind_actor', '切断② Actor 看不到指令'), ('no_height_rew', 'ablation_no_height_rew', '切断③ 不发高度奖励')]
(H_LO, H_HI) = (0.45, 0.8)
...
def main() -> int:
    ...
    if cal:
        print(f'── 指标校准 ──')
        print(f"  target_height_mean 实测 {tail_mean(runs['baseline'][0], 'Metrics/base_height/target_height_mean'):.4f}，真值应为 {(H_LO + H_HI) / 2:.4f}（指令均匀采样）")
        print(f'  → 指标被放大 {cal:.3f}× （metrics 累积整个 episode 却除以单条指令的最大步数）')
        print(f'  三组 episode 长度一致，故**相对比较不受影响**；下表给出校准后的绝对值。')
        print()
    print('── 主判据：高度跟踪误差（越小越好）──')
    print(f"  {'组别':<16}{'原始值':>12}{'校准后 m':>12}{'相对 baseline':>16}")
    base_err = tail_mean(runs['baseline'][0], 'Metrics/base_height/error_height') if 'baseline' in runs else None
    ...
    for (key, _, _) in GROUPS:
        ...
        if key == 'no_height_rew':
            note = '  ← 应为 0（权重已置零）' if r is not None and abs(r) < 1e-06 else '  ← ⚠️ 期望为 0'
        print(f'  {key:<16}{fmt(r):>24}{note}')
    print()
    marks = [0, 500, 1000, 1500, 2000, 2500, 2999]
    print('── error_height 收敛过程 ──')
    header = '  ' + f"{'iter':<8}" + ''.join((f'{k:>16}' for (k, _, _) in GROUPS if k in runs))
    print(header)
    series = {k: series_at(runs[k][0], 'Metrics/base_height/error_height', marks) for (k, _, _) in GROUPS if k in runs}
    ...
    if base_err is not None:
        worse = [(k, tail_mean(runs[k][0], 'Metrics/base_height/error_height')) for (k, _, _) in GROUPS if k in runs and k != 'baseline']
