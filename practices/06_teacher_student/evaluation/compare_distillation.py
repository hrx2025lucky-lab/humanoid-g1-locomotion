...
ROOT = os.environ.get('HW6_LOG_DIR', '/path/to/workspace/shenlan_hw/hw6_distill/logs/rsl_rl')
METHODS = [('action_matching', 'AC 对齐动作均值'), ('kl_matching', 'KL 对齐完整分布')]
VARIANTS = [('', '原始超参'), ('_aligned', '对齐超参')]
SKIP = {('kl_matching', '_aligned')}
HPARAMS = {('action_matching', ''): 'lr 5e-4 / ec 0.0025 / kl 0.005', ('kl_matching', ''): 'lr 3e-4 / ec 0.005  / kl 0.008', ('action_matching', '_aligned'): 'lr 3e-4 / ec 0.005  / kl 0.008', ('kl_matching', '_aligned'): 'lr 3e-4 / ec 0.005  / kl 0.008'}
METRICS = [('Metrics/motion/error_joint_pos', '关节位置误差', True), ('Metrics/motion/error_body_pos', '身体位置误差', True), ('Metrics/motion/error_body_rot', '身体姿态误差', True), ('Metrics/motion/error_anchor_pos', 'anchor 位置误差', True), ('Termination_Frac/fail', '失败占比', True), ('Train/mean_episode_length', 'episode 长度', False), ('Policy/mean_std', '策略噪声 std', None)]
...
def main() -> int:
    ...
    for (m, ml) in METHODS:
        for (v, vl) in VARIANTS:
            ...
            print(f"  {ml + ' · ' + vl:<28}{HPARAMS[key]:<32}{('—' if step is None else step):>8}{mark}")
    print()
    ...
    if ready:
        ...
        done = [f'{m}{v}' for ((m, v), r) in runs.items() if r[1] and r[1] >= TARGET_ITERS]
        ...
        if a0 and k0:
            ...
            for (tag, label, _) in METRICS[:5]:
                print(f'  {label:<20}{fmt(tail_mean(a0[0], tag)):>14}{fmt(tail_mean(k0[0], tag)):>14}')
        print('═' * 80)
    print()
    return 0
