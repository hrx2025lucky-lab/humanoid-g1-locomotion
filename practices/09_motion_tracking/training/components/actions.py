...
class ResidualJointPositionAction(BaseAction):
    ...
    def __init__(self, cfg: ResidualJointPositionActionCfg, env: ManagerBasedRlEnv):
        if cfg.use_default_offset:
            raise ValueError('ResidualJointPositionActionCfg requires use_default_offset=False because the motion command provides the position reference.')
        super().__init__(cfg=cfg, env=env)

    def apply_actions(self) -> None:
        command = cast(MotionCommand, self._env.command_manager.get_term(self.cfg.command_name))
...
class JointPositionVelocityAction(BaseAction):
    ...
    def __init__(self, cfg: JointPositionVelocityActionCfg, env: ManagerBasedRlEnv):
        ...
        self._vel_ref_dt = float(cfg.vel_ref_dt) if cfg.vel_ref_dt is not None else float(self._env.step_dt)
        ...
        self._has_prev_reference = torch.zeros(self.num_envs, dtype=torch.bool, device=self.device)
    ...
    def process_actions(self, actions: torch.Tensor):
        ...
        raw_velocity_reference = (position_reference - self._prev_position_reference) / self._vel_ref_dt
        first_step_mask = ~self._has_prev_reference
        raw_velocity_reference[first_step_mask] = 0.0
        alpha = self.cfg.vel_ref_alpha
        self._velocity_reference += alpha * (raw_velocity_reference - self._velocity_reference)
...
class ResidualJointPositionVelocityAction(JointPositionVelocityAction):
    ...
    def __init__(self, cfg: ResidualJointPositionVelocityActionCfg, env: ManagerBasedRlEnv):
        if cfg.use_default_offset:
            raise ValueError('ResidualJointPositionVelocityActionCfg requires use_default_offset=False because the motion command provides the position reference.')
        super().__init__(cfg=cfg, env=env)
