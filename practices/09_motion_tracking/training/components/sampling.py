...
import torch
from mjlab.utils.lab_api.math import quat_apply, quat_from_euler_xyz, quat_inv, quat_mul, sample_uniform, yaw_quat
if TYPE_CHECKING:
...
def cap_sampling_probabilities(probabilities: torch.Tensor, *, max_ratio: float) -> torch.Tensor:
  ...
  """
    ...
    if max_ratio <= 0.0:
        return probabilities / torch.clamp(probabilities.sum(), min=1e-08)
    probabilities = probabilities / torch.clamp(probabilities.sum(), min=1e-08)
    ...
    free = positive.clone()
    remaining_mass = torch.tensor(1.0, dtype=probabilities.dtype, device=probabilities.device)
    for _ in range(num_positive):
        base = probabilities[free]
        if base.numel() == 0:
            break
        candidate = base / torch.clamp(base.sum(), min=1e-08) * remaining_mass
    ...
    if torch.any(free):
        base = probabilities[free]
        capped[free] = base / torch.clamp(base.sum(), min=1e-08) * remaining_mass
    capped[~positive] = 0.0
    return capped / torch.clamp(capped.sum(), min=1e-08)
...
def adaptive_sampling(command: MotionCommand, env_ids: torch.Tensor) -> None:
    ...
    if command._uses_motion_library:
        ...
        if torch.any(episode_failed):
            ...
            failed_phase_bins = torch.clamp((failed_times / torch.clamp(failed_lengths, min=1e-06) * command.bin_count).long(), 0, command.bin_count - 1)
            command._current_motion_failed[:] = torch.bincount(failed_motion_ids, minlength=command.motion_lib.num_motions()).to(dtype=torch.float32)
            phase_flat_idx = failed_motion_ids * command.bin_count + failed_phase_bins
            command._current_phase_failed[:] = torch.bincount(phase_flat_idx, minlength=command.motion_lib.num_motions() * command.bin_count).to(dtype=torch.float32).reshape(command.motion_lib.num_motions(), command.bin_count)
        ...
        motion_probabilities = (1.0 - mix_ratio) * hard_motion_prob + mix_ratio * command.motion_lib.motion_weights
        horizon_s = _future_horizon_s(command)
        valid_mask = (command.motion_lib.motion_lengths_s > horizon_s + 1e-06).to(motion_probabilities.dtype)
        ...
        motion_probabilities = cap_sampling_probabilities(motion_probabilities, max_ratio=command.cfg.adaptive_max_motion_sampling_ratio)
        hard_phase_prob = command.phase_failed_count + 1.0 / float(command.bin_count)
        hard_phase_prob = torch.nn.functional.pad(hard_phase_prob.unsqueeze(1), (0, command.cfg.adaptive_kernel_size - 1), mode='replicate')
        hard_phase_prob = torch.nn.functional.conv1d(hard_phase_prob, command.kernel.view(1, 1, -1)).squeeze(1)
        hard_phase_prob = hard_phase_prob / torch.clamp(hard_phase_prob.sum(dim=1, keepdim=True), min=1e-08)
        uniform_phase_prob = torch.full_like(hard_phase_prob, 1.0 / float(command.bin_count))
        phase_probabilities = (1.0 - mix_ratio) * hard_phase_prob + mix_ratio * uniform_phase_prob
        phase_probabilities = phase_probabilities / torch.clamp(phase_probabilities.sum(dim=1, keepdim=True), min=1e-08)
        sampled_motion_ids = torch.multinomial(motion_probabilities, len(env_ids), replacement=True)
        ...
        sampled_phase = (sampled_bins + sample_uniform(0.0, 1.0, (len(env_ids),), device=command.device)) / command.bin_count
        sampled_lengths = command.motion_lib.get_motion_length(sampled_motion_ids)
        sampled_offsets = sampled_phase * sampled_lengths
        sampled_max_start = torch.clamp(sampled_lengths - horizon_s - 1e-06, min=0.0)
        sampled_offsets = torch.minimum(sampled_offsets, sampled_max_start)
        sampled_offsets = torch.minimum(sampled_offsets, torch.clamp(sampled_lengths - 1e-06, min=0.0))
        ...
        if command.motion_lib.num_motions() > 1:
            motion_entropy = -(motion_probabilities * (motion_probabilities + 1e-12).log()).sum() / math.log(command.motion_lib.num_motions())
        else:
            motion_entropy = torch.tensor(1.0, device=command.device)
        if command.bin_count > 1:
            phase_entropy = -(phase_probabilities * (phase_probabilities + 1e-12).log()).sum(dim=1).mean() / math.log(command.bin_count)
        else:
        ...
        command.metrics['sampling_motion_top1_idx'][:] = motion_top_idx.float() / command.motion_lib.num_motions()
    ...
    if torch.any(episode_failed):
        current_bin_index = torch.clamp(command.time_steps * command.bin_count // max(command.motion.time_step_total, 1), 0, command.bin_count - 1)
        fail_bins = current_bin_index[env_ids][episode_failed]
        command._current_bin_failed[:] = torch.bincount(fail_bins, minlength=command.bin_count)
    sampling_probabilities = command.bin_failed_count + command.cfg.adaptive_uniform_ratio / float(command.bin_count)
    sampling_probabilities = torch.nn.functional.pad(sampling_probabilities.unsqueeze(0).unsqueeze(0), (0, command.cfg.adaptive_kernel_size - 1), mode='replicate')
    sampling_probabilities = torch.nn.functional.conv1d(sampling_probabilities, command.kernel.view(1, 1, -1)).view(-1)
    sampling_probabilities = sampling_probabilities / sampling_probabilities.sum()
    sampled_bins = torch.multinomial(sampling_probabilities, len(env_ids), replacement=True)
    command.time_steps[env_ids] = ((sampled_bins + sample_uniform(0.0, 1.0, (len(env_ids),), device=command.device)) / command.bin_count * (command.motion.time_step_total - 1)).long()
...
def uniform_sampling(command: MotionCommand, env_ids: torch.Tensor) -> None:
    ...
    if command._uses_motion_library:
        ...
        horizon_s = _future_horizon_s(command)
        sampled_max_start = torch.clamp(sampled_lengths - horizon_s - 1e-06, min=0.0)
        command.motion_time_offsets[env_ids] = torch.rand(sampled_max_start.shape, device=command.device) * sampled_max_start
        command.time_steps[env_ids] = 0
        command._refresh_motion_frame()
        ...
        assert command.motion is not None
        max_start_idx = max(command.motion.time_step_total - 1 - _max_future_offset_steps(command), 0)
        command.time_steps[env_ids] = torch.randint(0, max_start_idx + 1, (len(env_ids),), device=command.device)
        command.metrics['sampling_motion_entropy'][:] = 0.0
        command.metrics['sampling_motion_top1_prob'][:] = 0.0
        command.metrics['sampling_motion_top1_idx'][:] = 0.0
        command.metrics['sampling_phase_entropy'][:] = 1.0
...
def reset_robot_to_reference(command: MotionCommand, env_ids: torch.Tensor) -> None:
    ...
    range_list = [command.cfg.pose_range.get(key, (0.0, 0.0)) for key in ['x', 'y', 'z', 'roll', 'pitch', 'yaw']]
    ranges = torch.tensor(range_list, device=command.device)
    rand_samples = sample_uniform(ranges[:, 0], ranges[:, 1], (len(env_ids), 6), device=command.device)
    root_pos += rand_samples[:, 0:3]
    orientations_delta = quat_from_euler_xyz(rand_samples[:, 3], rand_samples[:, 4], rand_samples[:, 5])
    root_ori = quat_mul(orientations_delta, root_ori)
    range_list = [command.cfg.velocity_range.get(key, (0.0, 0.0)) for key in ['x', 'y', 'z', 'roll', 'pitch', 'yaw']]
    ranges = torch.tensor(range_list, device=command.device)
    rand_samples = sample_uniform(ranges[:, 0], ranges[:, 1], (len(env_ids), 6), device=command.device)
    ...
    joint_pos += sample_uniform(lower=command.cfg.joint_position_range[0], upper=command.cfg.joint_position_range[1], size=joint_pos.shape, device=joint_pos.device)
...
def update_command(command: MotionCommand) -> None:
    ...
    if command.cfg.sampling_mode == 'adaptive':
        if command._uses_motion_library:
            ...
            assert command.phase_failed_count is not None
            assert command._current_phase_failed is not None
            command.motion_failed_count = command.cfg.adaptive_alpha * command._current_motion_failed + (1 - command.cfg.adaptive_alpha) * command.motion_failed_count
            command.phase_failed_count = command.cfg.adaptive_alpha * command._current_phase_failed + (1 - command.cfg.adaptive_alpha) * command.phase_failed_count
            command._current_motion_failed.zero_()
            command._current_phase_failed.zero_()
        else:
            assert command.bin_failed_count is not None
            assert command._current_bin_failed is not None
            command.bin_failed_count = command.cfg.adaptive_alpha * command._current_bin_failed + (1 - command.cfg.adaptive_alpha) * command.bin_failed_count
            command._current_bin_failed.zero_()
