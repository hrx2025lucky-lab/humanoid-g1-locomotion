...
**关于画面上的顿挫：** 回放中足端看起来一顿一顿，来自参考动作本身而非策略。同一段、同一口径下，机器人足端的加加速度p95为左456、右420，而它跟踪的参考动作为左487、右520——机器人比参考更平滑。支撑期足端水平速度中位为0.000m/s，无滑步。扩容训练把足端跟踪RMSE由0.131m降到0.045m、最大偏差由0.345m降到0.097m，改善的是"跟得准不准"；"看起来顺不顺"受参考动作限制，不是训练量能解决的。
