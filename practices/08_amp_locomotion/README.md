...
- 用八轮单因素实验定位摆臂问题，每轮判据在训练开始前写入冻结方案：已排除延长训练、风格与任务配比、关节速度惩罚、框架自带的接触时序奖励四条路径。
- 找到有效改动：**将判别器梯度惩罚由10提高到50—100，摆臂方向得到修正**。该结果在新随机种子下复现，并在更高剂量下保持。
...
**验证范围：**冻结门槛要求0.3m/s慢走与0.5m/s前进两个场景同时通过，无模型达成，因此不宣称自然步态达成。具体未解决两项：一是双支撑占比43.8%—45.6%，与基线44.3%基本相同，正常人常速行走为20%—25%，走走停停的节奏没有改善，画面上仍能看出顿挫，且整个周期腾空占比为0；二是0.3m/s慢走全部卡在摆腿抬升不足，参考动作库中0.2—0.45m/s的最长连续片段只有0.40秒，属数据缺口而非调参可解。另外，此前用于衡量摆臂的"肩关节活动范围"只测幅度不测相位，无法区分自然摆臂与同手同脚；相位指标与双支撑指标均登记为后续实验使用，不回溯套用于已完成轮次的判定。
