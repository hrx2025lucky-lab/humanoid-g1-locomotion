...
GROUPS = [
    ("baseline", "unitree_g1_29dof_navigation_hrl_baseline", "固定竞技场"),
    ("random_arena", "unitree_g1_29dof_navigation_hrl_randomarena", "每 episode 随机重排障碍"),
]
...
# 真实任务指标：导航到底做到没有
TASK = [
    ("Metrics/pose_command/goals_reached", "到达目标数", False),
...
def main() -> int:
    ...
    for key, subdir, _ in GROUPS:
        matches = [d for d in sorted(glob.glob(os.path.join(ROOT, subdir, "*")))
                   if os.path.isdir(d)]
        if not matches:
            print(f"⚠️  找不到 {key} 的 run（{subdir}）", file=sys.stderr)
            continue
        ea = load(matches[-1])
        if ea is not None:
    ...
    print("── 表面指标（好看不代表学会了）──")
    print(f"  {'指标':<22}" + "".join(f"{k:>18}" for k, _, _ in GROUPS if k in runs))
    ...
    for tag, label, lower_better in TASK:
        vals = {k: tail_mean(runs[k][0], tag) for k, _, _ in GROUPS if k in runs}
        row = f"  {label:<22}"
        for key, _, _ in GROUPS:
            if key in runs:
    ...
    for key, _, _ in GROUPS:
        ...
        goals = tail_mean(ea, "Metrics/pose_command/goals_reached")
    ...
    if not healthy:
        ...
        b = tail_mean(runs["baseline"][0], "Metrics/pose_command/goals_reached") \
            if "baseline" in runs else None
        r = tail_mean(runs["random_arena"][0], "Metrics/pose_command/goals_reached") \
            if "random_arena" in runs else None
        if b is not None and r is not None:
            print(f"两组均在学习。到达目标数：baseline {b:.4f} vs random_arena {r:.4f}")
            print("随机重排障碍通常收敛更慢但泛化更好 —— 判断时要看曲线趋势而非单点。")
    print("═" * 76)
    print()
    return 0
