...
# 正式训练用 4096，不能沿用冒烟的 512。
#
# 这里原本写的是 ${NUM_ENVS:-512}，而 512 是冒烟测试的规模——
# 冒烟只要验证"跑得起来"，正式训练要的是样本量。后果很实在：
# 课程给的训练预算折算是 Env × Iter ≈ 4096 × 10000，
#   4096→10k    2048→20k    1024→40k    512→80k
# 用 512 跑 3000 轮 = 官方基准的 3.75%，地形等级只爬到 1.28/9 就停了。
# 而且当时还误以为"3000 轮跑得真快"，其实是环境数少了 8 倍。
#
# 4096 已实测可行：9-7 那次用 4096 跑了 616 轮，全程无 OOM。
# 冒烟阶段若真的 OOM 过（NUM_ENVS 被改成 256），这里会沿用那个值。
NUM_ENVS="${NUM_ENVS:-4096}"

# ── 正式训练 ────────────────────────────────────────────────
# 轮数怎么定：课程给的折算是 Env × Iter ≈ 4096 × 10000。
# 4096 环境要跑满就是 10000 轮，按独占 GPU 的实测速度约 14~22 小时。
# 先跑 3000 轮（= 官方基准 30%，与实践 4/5/6/8 同口径，那几个都已收敛），
# 跑完看 terrain_levels 还在不在爬，再决定要不要续。
ITERS="${P11_ITERS:-3000}"
